...
if __name__ == "__main__":
    ...
    if os.path.exists(bm_file):
        ...
        report = DataValidator.validate_bank_marketing(df_bm)
        print("Bank Marketing Validation:", report)
        if not report["success"]:
            all_valid = False
        
    if os.path.exists(comp_file):
        df_comp = pd.read_csv(comp_file)
        report = DataValidator.validate_complaints(df_comp)
        print("Complaints Validation:", report)
        if not report["success"]:
            all_valid = False
